...
def build_artifacts(*, boundary_enabled: bool, boundary_entropy: float | None):
    summary = {
        "available": True,
        "global_entropy": 0.25,
...
class UncertaintyContractTests(unittest.TestCase):
    ...
    def test_entropy_only_checkpoint_marks_boundary_unavailable(self):
        payload = self.build_payload(
            build_artifacts(boundary_enabled=False, boundary_entropy=None)
        )
        ...
        self.assertNotIn("risk_control", payload)
        self.assertNotIn("conformal", payload)

    def test_boundary_enabled_checkpoint_exposes_hb(self):
        payload = self.build_payload(
            build_artifacts(boundary_enabled=True, boundary_entropy=0.4)
        )
    ...
    def test_empty_predicted_boundary_is_not_presented_as_meaningful_hb(self):
        artifacts = build_artifacts(boundary_enabled=True, boundary_entropy=0.0)
        artifacts = replace(
            artifacts,
            extra_metadata={
